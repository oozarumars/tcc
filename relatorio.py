...
## Este programa gera os gráficos, histogramas e dados estatísticos.
...
def executar():
    ...
    #global inicio_medicao, final_medicao, elev, afun, interrup, interrupa, interrup, interrupc# rascunho
    ...
    class fase(): #classe a ser instanciada para cada fase
        tensao_ref = 220
        ltpi = 189.0
        ltai = 201.0
        ltas = 231.0
        ...
        def calcular_drp(self): #calcula o drp de uma fase
            n = 0
        ...
        def calcular_drc(self): #calcula o drc de uma fase
            n = 0
            for j in range(len(self.dados_rms)):
        ...
        def dados_histograma(self): #organizar dados para o histograma
            k = 0.80
            for i in range(40):
                if i == 0:
                    self.dici = {"< 0.80": 0}
    ...
    class relatorio(): #gerar gráfico, histograma e demais dados estátisticos 
        ...
        def graf_trp(self, menor_trp, maior_trp):
            leitura = []
            for i in range(qtd_leituras): #cria uma lista para associar os valores de tensão obtidos e plotar
                leitura.append(i+1)
            ...
            plt.fill_between(leitura, [fase.ltps]*(qtd_leituras), [1000]*(qtd_leituras), alpha = 0.3, color = "orange", label = "Crítica")
            plt.fill_between(leitura, [0]*(qtd_leituras), [fase.ltpi]*(qtd_leituras), alpha = 0.3, color = "orange")
            plt.legend(ncol = 5, columnspacing = 0.2, fontsize = 10, loc = 'upper center')
            plt.grid()
            plt.xlim(1, qtd_leituras)
            plt.ylim(menor_trp, maior_trp) #personaliza o eixo vertical do gráfico
        ...
        def graf_histograma(self): #configuraçãos do histograma
        ...
        def maior_drp(self): #calcular maior drp entre as fases
            self.drp = 0.0
            self.lista_drp = []
        ...
        def maior_drc(self): #calcular maior drc entre as fases
            self.drc = 0
            self.lista_drc = []
            if qtd_fases == 1 or qtd_fases == 2 or qtd_fases == 3:
        ...
        def gerar_relatorio(self): #gera um relatório simples em txt 
            try:
    ...
    dados.graf_trp(min([f1_menor, f2_menor, f3_menor])-3, max([f1_maior, f2_maior, f3_maior])+3)
